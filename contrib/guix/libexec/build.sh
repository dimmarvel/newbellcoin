#!/usr/bin/env bash
# Copyright (c) 2019-2022 The bellscoin Core developers
...
(
    ...
    case "$HOST" in
        *mingw*)
            make deploy ${V:+V=1} BELLSCOIN_WIN_INSTALLER="${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our bellscoin Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built bellscoin Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example bellscoin.conf file. if contrib/devtools/gen-bellscoin-conf.sh
