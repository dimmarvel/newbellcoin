# Bootstrappable bellscoin Core Builds

This directory contains the files necessary to perform bootstrappable bellscoin
Core builds.
...
To build bellscoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      bellscoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/bellscoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/bellscoin-core/guix.sigs).
