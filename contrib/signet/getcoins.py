#!/usr/bin/env python3
# Copyright (c) 2020-2022 The bellscoin Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bellscoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bellscoin-cli', help='bellscoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='bellscoin address to which the faucet should send')
...
parser.add_argument('bellscoin_cli_args', nargs='*', help='Arguments to pass on to bellscoin-cli (default: -signet)')

args = parser.parse_args()

if args.bellscoin_cli_args == []:
    args.bellscoin_cli_args = ['-signet']


def bellscoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bellscoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bellscoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bellscoin_cli(['getnewaddress', 'faucet', 'bech32'])
