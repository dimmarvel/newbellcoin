...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<BellscoinUnit>("BellscoinUnit");
#else
    qRegisterMetaType<BellscoinUnit>("BellscoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bellscoin_de.qm (shortcut "de" needs to be defined in bellscoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bellscoin_de_DE.qm (shortcut "de_DE" needs to be defined in bellscoin.qrc)
...
static const char* qt_argv = "bellscoin-qt";

BellscoinApplication::BellscoinApplication()
...
void BellscoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BellscoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BellscoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BellscoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BellscoinApplication::createOptionsModel(bool resetSettings)
{
...
void BellscoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BellscoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BellscoinGUI::quitRequested, this, &BellscoinApplication::requestShutdown);
...
void BellscoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BellscoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BellscoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BellscoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BellscoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BellscoinApplication::handleRunawayException);
    connect(this, &BellscoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BellscoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BellscoinApplication::parameterSetup()
{
...
void BellscoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BellscoinApplication::requestInitialize()
{
...
void BellscoinApplication::requestShutdown()
{
...
void BellscoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // bellscoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BellscoinGUI::handlePaymentRequest);
            connect(window, &BellscoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BellscoinApplication::handleRunawayException(const QString &message)
{
...
void BellscoinApplication::handleNonFatalException(const QString& message)
{
...
WId BellscoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BellscoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BellscoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(bellscoin);
    Q_INIT_RESOURCE(bellscoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(BELLSCOIN_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see bellscoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, PACKAGE_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see bellscoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    // bellscoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
