...
/** Call node.updateRwSetting() with bellscoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because bellscoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/bellscoin/bellscoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because bellscoin 22.x will not try to read these.
...
//! Convert enabled/size values to bellscoin -prune setting.
...
//! Get pruning enabled value to show in GUI from bellscoin -prune setting.
...
//! Get pruning size value to show in GUI from bellscoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayBellscoinUnit")) {
        settings.setValue("DisplayBellscoinUnit", QVariant::fromValue(BellscoinUnit::BELLS));
    }
    QVariant unit = settings.value("DisplayBellscoinUnit");
    if (unit.canConvert<BellscoinUnit>()) {
        m_display_bellscoin_unit = unit.value<BellscoinUnit>();
    } else {
        m_display_bellscoin_unit = BellscoinUnit::BELLS;
        settings.setValue("DisplayBellscoinUnit", QVariant::fromValue(m_display_bellscoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if bellscoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_bellscoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BellscoinUnit>() == m_display_bellscoin_unit) return;
    m_display_bellscoin_unit = new_unit.value<BellscoinUnit>();
    QSettings settings;
    settings.setValue("DisplayBellscoinUnit", QVariant::fromValue(m_display_bellscoin_unit));
    Q_EMIT displayUnitChanged(m_display_bellscoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/bellscoin/bellscoin/pull/8273
    ...
    // (https://github.com/bellscoin-core/gui/issues/567).
