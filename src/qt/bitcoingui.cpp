...
const std::string BellscoinGUI::DEFAULT_UIPLATFORM =
...
BellscoinGUI::BellscoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &BellscoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &BellscoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &BellscoinGUI::showModalOverlay);
...
void BellscoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a bellscoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and bellscoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &BellscoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &BellscoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &BellscoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your bellscoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified bellscoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed bellscoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed bellscoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a bellscoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible bellscoin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &BellscoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &BellscoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &BellscoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &BellscoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &BellscoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &BellscoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &BellscoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &BellscoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &BellscoinGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &BellscoinGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &BellscoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &BellscoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &BellscoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &BellscoinGUI::showDebugWindow);
}

void BellscoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void BellscoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &BellscoinGUI::setCurrentWalletBySelectorIndex);
...
void BellscoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &BellscoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &BellscoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &BellscoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &BellscoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void BellscoinGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void BellscoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &BellscoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &BellscoinGUI::removeWallet);
...
WalletController* BellscoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void BellscoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &BellscoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &BellscoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &BellscoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &BellscoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &BellscoinGUI::incomingTransaction);
    connect(this, &BellscoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void BellscoinGUI::removeWallet(WalletModel* walletModel)
{
...
void BellscoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void BellscoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void BellscoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BellscoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void BellscoinGUI::createTrayIcon()
{
...
void BellscoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &BellscoinGUI::toggleHidden);
...
void BellscoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void BellscoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void BellscoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void BellscoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void BellscoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void BellscoinGUI::openClicked()
{
...
void BellscoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BellscoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BellscoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BellscoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BellscoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BellscoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void BellscoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void BellscoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to bellscoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void BellscoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void BellscoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void BellscoinGUI::updateHeadersSyncProgressLabel()
{
...
void BellscoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void BellscoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &BellscoinGUI::quitRequested);
...
void BellscoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void BellscoinGUI::createWallet()
{
#ifdef ENABLE_WALLET
#ifndef USE_SQLITE
    ...
    connect(activity, &CreateWalletActivity::created, this, &BellscoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void BellscoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void BellscoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &BellscoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &BellscoinGUI::show);
                e->ignore();
            }
        }
    }
...
void BellscoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BellscoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BellscoinGUI::incomingTransaction(const QString& date, BellscoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(BellscoinUnits::formatWithUnit(unit, amount, true));
...
void BellscoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BellscoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BellscoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BellscoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BellscoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void BellscoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void BellscoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void BellscoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void BellscoinGUI::updateWindowTitle()
{
...
void BellscoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BellscoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BellscoinGUI::detectShutdown()
{
...
void BellscoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void BellscoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(BellscoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = BellscoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void BellscoinGUI::subscribeToCoreSignals()
{
...
void BellscoinGUI::unsubscribeFromCoreSignals()
{
...
bool BellscoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<BellscoinUnit> units = BellscoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const BellscoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, BellscoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const BellscoinUnit u : BellscoinUnits::availableUnits()) {
        menu->addAction(BellscoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(BellscoinUnit newUnits)
{
    setText(BellscoinUnits::longName(newUnits));
}
