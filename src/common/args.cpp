...
const char * const BELLSCOIN_CONF_FILENAME = "bellscoin.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bellscoin-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\bellscoin
    // macOS: ~/Library/Application Support/bellscoin
    // Unix-like: ~/.bellscoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "bellscoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/bellscoin";
#else
    // Unix-like
    return pathRet / ".bellscoin";
#endif
#endif
}
