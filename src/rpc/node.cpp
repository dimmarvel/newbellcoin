...
static RPCHelpMan echo(const std::string& name)
{
    return RPCHelpMan{name,
                ...
                "bellscoin-cli and the GUI. There is no server-side difference.",
        {
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                // Spawn a new bellscoin-node process and call makeEcho to get a
                // client pointer to a interfaces::Echo instance running in
                // that process. This is just for testing. A slightly more
                // realistic test spawning a different executable instead of
                // the same executable would add a new bellscoin-echo executable,
                // and spawn bellscoin-echo below instead of bellscoin-node. But
                // using bellscoin-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("bellscoin-node");
                echo = init->makeEcho();
