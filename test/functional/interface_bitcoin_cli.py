...
# The block reward of coinbaseoutput.nValue (50) BELLS/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 BELLS/block.
...
class TestBellscoinCli(BellscoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `bellscoin-cli getblockchaininfo`")
        ...
        self.log.info("Test -getinfo and bellscoin-cli return all proxies")
        ...
        if self.is_specified_wallet_compiled():
            self.log.info("Test -getinfo and bellscoin-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (BELLS/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (BELLS/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 BELLS) to the default wallet.
            ...
            # Test bellscoin-cli -generate with -rpcwallet in multiwallet mode.
