...
TMPDIR_PREFIX = "bellscoin_func_test_"
...
class BellscoinTestMetaClass(type):
    """Metaclass for BellscoinTestFramework.

    Ensures that any attempt to register a subclass of `BellscoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'BellscoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("BellscoinTestFramework subclasses must override "
...
class BellscoinTestFramework(metaclass=BellscoinTestMetaClass):
    """Base class for a bellscoin test script.

    Individual bellscoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use bellscoin-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "bellscoind": ("bellscoind", "BELLSCOIND"),
            "bellscoin-cli": ("bellscoincli", "BELLSCOINCLI"),
            "bellscoin-util": ("bellscoinutil", "BELLSCOINUTIL"),
            "bellscoin-wallet": ("bellscoinwallet", "BELLSCOINWALLET"),
        }
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("BellscoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'bellscoin-cli', self.options.bellscoincli) for v in versions]
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the bellscoind
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    bellscoin_cli=self.options.bellscoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in bellscoin.conf
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if bellscoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("bellscoin-wallet has not been compiled")

    def skip_if_no_bellscoin_util(self):
        """Skip the running test if bellscoin-util has not been compiled."""
        if not self.is_bellscoin_util_compiled():
            raise SkipTest("bellscoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if bellscoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("bellscoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether bellscoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether bellscoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_bellscoin_util_compiled(self):
        """Checks whether bellscoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_BELLSCOIN_UTIL")
