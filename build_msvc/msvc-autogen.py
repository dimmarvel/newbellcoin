...
def parse_config_into_bells_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"bellscoin Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/bellscoin_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/bellscoin_config.h'), "w", encoding="utf8") as bells_config:
        bells_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='bellscoin-core msbuild configuration initialiser.')
    ...
    parse_config_into_bells_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/bellscoin_config.h'), os.path.join(SOURCE_DIR, 'config/bellscoin-config.h'))
