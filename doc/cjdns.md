# CJDNS support in bellscoin Core

It is possible to run bellscoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of bellscoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the bellscoin network and individual nodes.
...
before it will work with your bellscoin Core node. This is described in steps
...
## Run bellscoin Core with CJDNS

Once you are connected to the CJDNS network, the following bellscoin Core
...
When enabled, this option tells bellscoin Core that it is running in an
...
IPv6 local network. This helps bellscoin Core perform better address management:
...
CJDNS support was added to bellscoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bellscoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/bellscoin/bellscoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in bellscoin Core

There are several ways to see your CJDNS address in bellscoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bellscoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bellscoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `bellscoin-cli help getnodeaddresses` for details.
