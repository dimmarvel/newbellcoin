# PSBT Howto for bellscoin Core

Since bellscoin Core 0.17, an RPC interface exists for Partially Signed bellscoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/bellscoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for bellscoin transactions that are not fully signed
...
#### Multisig with multiple bellscoin Core instances
...
bellscoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bellscoin-cli` is used.
...
Later, when *V* BELLS has been received on *Amulti*, and Bob and Carol want to
