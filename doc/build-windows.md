...
Below are some notes on how to build bellscoin Core for Windows.

The options known to work for building bellscoin Core on Windows are:
...
Note that for WSL the bellscoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/bellscoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\bellscoin`, for example:

    make install DESTDIR=/mnt/c/workspace/bellscoin
