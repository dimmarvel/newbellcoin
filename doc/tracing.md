# User-space, Statically Defined Tracing (USDT) for bellscoin Core

bellscoin Core includes statically defined tracepoints to allow for more
...
**Note**: bellscoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to bellscoin Core
...
Multiple tools can list the available tracepoints in a `bellscoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in bellscoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/bellscoind
stap net        outbound_message 0x0000000000107c05 /src/bellscoind
stap validation block_connected  0x00000000002fb10c /src/bellscoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in bellscoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/bellscoind | grep NT_STAPSDT -A 4 -B 2
