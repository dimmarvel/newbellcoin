...
bellscoin Core version 0.20.0 is now available from:

  <https://bellscoincore.org/bin/bellscoin-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `BELLSCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from bellscoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since bellscoin Core version 0.18.0.
...
- Testing or debugging of implementations of the bellscoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of bellscoin Core. bellscoin Core logs debug messages
...
- OpenSSL is no longer used by bellscoin Core.  (#17265)

- BIP70 support has been fully removed from bellscoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/bellscoin/bellscoin/issues/16599),
  [PR #16702](https://github.com/bellscoin/bellscoin/pull/16702), and the
  `bellscoind help` for more information.  This option is experimental and
...
- All custom settings configured when bellscoin Core starts are now
...
- The "Start bellscoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bellscoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple bellscoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed bellscoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bellscoin-core/HWI) for
  signing.  Future versions of bellscoin Core should support a GUI option
...
- bellscoin Core's automatically-generated source code documentation is
  now available at https://doxygen.bellscoincore.org.  (#17596)
...
- The `bellscoin-cli` utility used with the `-getinfo` parameter now
...
- The `bellscoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `bellscoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_bellscoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBBELLSCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_bellscoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBBELLSCOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for bellscoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate bellscoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bellscoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_bellscoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (bellscoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone bellscoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about bellscoin Core files (hebasto)
...
- #17119 Fix broken bellscoin-cli examples (andrewtoth)
...
- #17596 Change doxygen URL to doxygen.bellscoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to bellscoin-wallet (hel-o)
...
- #18341 Replace remaining literal BELLS with `CURRENCY_UNIT` (domob1812)
...
- #16115 On bellscoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from bellscoin-cli -getinfo (malevolent)
